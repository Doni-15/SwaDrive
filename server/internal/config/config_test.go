...
func TestLoadServerRequiresPathsAndParsesOverrides(t *testing.T) {
	...
	if configuration.DatabasePath != "/tmp/state.db" || configuration.StorageRoot != "/tmp/storage" {
		t.Fatalf("paths = %q, %q; want configured paths", configuration.DatabasePath, configuration.StorageRoot)
	}
